...
class ChemicalService:
    ...
    def create_chemical(chemicals):
        ...
        try:
            ...
            db.session.rollback()
            raise Exception({"statusCode": 409, "message": "Quimico {} já cadastrado".format(ChemicalService.get_field_name(err.orig.args[0]))})
        except Exception as e:
            db.session.rollback()
            logger.error(e, exc_info=True)
            raise Exception({"statusCode": 500, "message": "Error ao salvar quimico"})
    ...
    def update_chemical(chemical_id, name):
        ...
        if not chemical:
            raise Exception({"statusCode": 404, "message": "Não existe quimico com id {}".format(chemical_id)})

        try:
            chemical.update_chemical(name)
        except IntegrityError:
            raise Exception({"statusCode": 409, "message": "Quimico já cadastrado"})
        except Exception as e:
            logger.error(e, exc_info=True)
            raise Exception({"statusCode": 500, "message": "Error ao alterar quimico"})
