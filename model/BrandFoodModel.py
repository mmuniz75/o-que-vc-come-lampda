...
class BrandFoodModel(db.Model):
    ...
    def find_by_food(cls, food_id):
        return cls.query.filter_by(id_food=food_id).order_by(unaccent(text("food_1.name")))

    @classmethod
    def find_by_brand(cls, brand_id):
        return cls.query.filter_by(id_brand=brand_id).order_by(unaccent(text("brand_1.name")))
